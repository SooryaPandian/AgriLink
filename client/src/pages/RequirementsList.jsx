...
export default function RequirementsList() {
  ...
  const filtered = requirements.filter(
    (r) => r.cropName?.toLowerCase().includes(search.toLowerCase()) || r.targetRegion?.toLowerCase().includes(search.toLowerCase())
  );
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-extrabold" style={{ color: "var(--text)" }}>Crop Requirements</h1>
          <p className="text-sm mt-1" style={{ color: "var(--text-muted)" }}>Manage your posted requirements and track applications</p>
        </div>
      ...
      {filtered.length === 0 ? (
        <div className="rounded-2xl border flex flex-col items-center py-20" style={{ background: "var(--bg-card)", borderColor: "var(--border)" }}>
          ...
          <p className="text-sm mb-6" style={{ color: "var(--text-muted)" }}>Post your first crop requirement to connect with farmers.</p>
          <Link to="/requirements/new" className="flex items-center gap-2 px-5 py-2.5 rounded-xl font-semibold text-white bg-gradient-to-r from-green-600 to-green-500 hover:from-green-700 hover:to-green-600 transition-all shadow-lg shadow-green-900/30 text-sm">
        ...
        <div className="rounded-2xl border overflow-hidden" style={{ borderColor: "var(--border)" }}>
          <div className="overflow-x-auto">
            <table className="w-full border-collapse text-sm">
              ...
              <tbody>
                {filtered.map((r, i) => (
                  <tr key={r._id} className="transition-colors" style={{ borderBottom: "1px solid var(--border)", background: i % 2 === 0 ? "var(--bg-card)" : "var(--bg)" }}>
                    ...
                    <td className="px-4 py-3" style={{ color: "var(--text-muted)" }}>{r.targetRegion || "�"}</td>
                    <td className="px-4 py-3 font-semibold text-amber-500">₹{r.initialPriceExpectation || "–"}</td>
                    <td className="px-4 py-3">
                      <span className={`font-bold ${r.applicationCount > 0 ? "text-green-500" : ""}`} style={r.applicationCount === 0 ? { color: "var(--text-dim)" } : {}}>
                    ...
                    <td className="px-4 py-3 text-xs" style={{ color: "var(--text-dim)" }}>{new Date(r.createdAt).toLocaleDateString()}</td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <Link to={`/requirements/${r._id}`} className="flex items-center gap-1 px-3 py-1.5 rounded-lg border text-xs font-semibold transition-all hover:border-green-500/50" style={{ borderColor: "var(--border)", color: "var(--text-muted)" }}>
                          <Eye size={13} /> View
                        </Link>
                        {r.status === "open" && r.applicationCount > 0 && (
                          <button onClick={() => startNegotiation(r._id)} className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-semibold text-white bg-amber-500 hover:bg-amber-600 transition-all">
                            <Play size={13} /> Negotiate
                          </button>
                        )}
                        {r.status === "negotiating" && (
                          <Link to={`/requirements/${r._id}/negotiate`} className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-semibold text-white bg-green-600 hover:bg-green-700 transition-all">
