...
export default function NegotiationsListScreen({ navigation }) {
  ...
  const fetch = useCallback(async () => {
    try {
      const params = {};
      if (search) params.crop = search;3
      const { data } = await api.get('/farmers/requirements/negotiating', { params });
